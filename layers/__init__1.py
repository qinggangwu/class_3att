...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    if cfg.MODEL.IF_LABELSMOOTH == 'on':
        xent = CrossEntropyLabelSmooth(num_classes=num_classes, device=device)     # new add by luo
        print("label smooth on, numclasses:", num_classes)

    if sampler == 'softmax' or cfg.MODEL.METRIC_LOSS_TYPE == 'softmax':
        ...
        def loss_func(score, feat, target):
            ...
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                ...
                #print('get ms loss')
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    return xent(score, target) + msloss(feat, target)  # new add by luo, open label smooth
                else:
                    return F.cross_entropy(score, target) + msloss(feat, target)    # new add by luo, no label smooth
            elif cfg.MODEL.METRIC_LOSS_TYPE == 'cluster':
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
...
def make_loss_encoder(cfg):
    def loss_func(feat, decode_feat):
        return torch.nn.MSELoss()(feat, decode_feat)

    return loss_func


def make_loss_with_center(cfg, num_classes, feat_size=2048):    # modified by gu
    device = cfg.MODEL.DEVICE
    ...
    if cfg.MODEL.METRIC_LOSS_TYPE == 'center':
        center_criterion = CenterLoss(num_classes=num_classes, feat_dim=feat_dim, device=device, use_gpu=True)  # center loss

    elif cfg.MODEL.METRIC_LOSS_TYPE == 'range_center':
        center_criterion = CenterLoss(num_classes=num_classes, feat_dim=feat_dim, device=device, use_gpu=True)  # center_range loss
