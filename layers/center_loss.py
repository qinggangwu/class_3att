...
class CenterLoss(nn.Module):
    ...
    def __init__(self, num_classes=751, feat_dim=2048, device="cuda:0", use_gpu=True):
        ...
        if self.use_gpu:
            self.centers = nn.Parameter(torch.randn(self.num_classes, self.feat_dim).cuda(self.device))
        else:
    ...
    def forward(self, x, labels):
        ...
        if self.use_gpu: classes = classes.cuda(self.device)
